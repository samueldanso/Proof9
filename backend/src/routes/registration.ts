...
import { createCommercialRemixTerms, NFTContractAddress, SPGNFTContractAddress } from '../../utils/utils'
...
app.post('/register', zValidator('json', RegistrationSchema), async (c) => {
    try {
        const { ipMetadata: ipMetadataInput, nftMetadata, commercialRemixTerms } = c.req.valid('json')
        ...
        const terms = commercialRemixTerms || { defaultMintingFee: 1, commercialRevShare: 5 }
 ...
 */
app.post('/register-custom', zValidator('json', RegistrationSchema), async (c) => {
    try {
        const { ipMetadata: ipMetadataInput, nftMetadata, commercialRemixTerms } = c.req.valid('json')
