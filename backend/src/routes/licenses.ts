...
const OneTimeUseLicenseSchema = z.object({
  ...
  licenseTemplate: z.string().default('0x2E896b0b2Fdb7457499B56AAaA4AE55BCB4Cd316')
})
...
app.post('/mint', zValidator('json', MintLicenseSchema), async (c) => {
  try {
    const { licensorIpId, licenseTermsId, amount, maxMintingFee, maxRevenueShare } = c.req.valid('json')
 ...
 */
app.post('/one-time-use', zValidator('json', OneTimeUseLicenseSchema), async (c) => {
  try {
    const { metadata, commercialRemixTerms, licenseTokenLimit, licenseTemplate } = c.req.valid('json')
    ...
    const ipMetadata = {
      ...
      ipMetadataHash: metadata.ipMetadataHash || toHex('test-metadata-hash', { size: 32 }),
      nftMetadataHash: metadata.nftMetadataHash || toHex('test-nft-metadata-hash', { size: 32 }),
