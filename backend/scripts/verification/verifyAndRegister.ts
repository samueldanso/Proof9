...
const main = async () => {
  try {
    ...
    const verificationResult = await verifyMusicContent(
      title,
      description,
      creatorId,
      mediaUrl,
    ...
    const hasInfringements = verificationResult.verificationStatus.some(
      (status) => {
        return (
    ...
    if (hasInfringements) {
      ...
      console.log(
        "Please review the verification results and resolve any issues.",
      )
      return
    }
